...
Compose uses `.env` for both interpolation and the server container environment. The release
controls the internal container port, database path and static-asset path; do not override `PORT`,
...
Use a unique Compose project name, port and volume for every additional instance.
...
| `SKYTRACE_COVERAGE_CELL_HORIZONTAL_STEP_NM` | `1` | positive finite number |
| `SKYTRACE_COVERAGE_CELL_VERTICAL_STEP_FT` | `400` | positive finite number |
