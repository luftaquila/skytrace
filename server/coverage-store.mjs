...
function normalizeOptions(raw = {}) {
  ...
  const configKey = crypto.createHash("sha256").update(JSON.stringify({
    coverageWindowHours,
    cellHorizontalStepNm,
    cellVerticalStepFt,
    maxSegmentSeconds,
    maxSegmentNm,
    maxSegmentAltitudeFt,
    maxSegmentSteps: MAX_SEGMENT_STEPS,
    datumDegrees: 0.5,
  })).digest("hex");
  return {
    ...raw,
...
export function snapCoverageDatum(lat, lon) {
  const snappedLat = clamp(Math.round(Number(lat) * 2) / 2, -90, 90);
  const roundedLon = Math.round(Number(lon) * 2) / 2;
  const snappedLon = ((roundedLon + 180) % 360 + 360) % 360 - 180;
  return { lat: snappedLat, lon: snappedLon };
}

function stateOrigin(db, receiver, cutoff, now) {
  const receiverLat = finiteNumber(receiver.lat);
  const receiverLon = finiteNumber(receiver.lon);
  if (receiverLat != null && receiverLon != null) return snapCoverageDatum(receiverLat, receiverLon);
  const first = db.prepare(`
    ...
    LIMIT 1
  `).get(receiver.id, cutoff, now);
  return first ? snapCoverageDatum(first.lat, first.lon) : null;
}
...
function ensureReceiverState(db, receiver, cutoff, options, now) {
  const current = db.prepare(`
    SELECT receiver_id, config_key, origin_lat, origin_lon, last_track_id
  ...
  const configuredOrigin = receiverLat != null && receiverLon != null
    ? snapCoverageDatum(receiverLat, receiverLon)
    : null;
  const originChanged = configuredOrigin && current
    ...
    : false;
  if (current?.config_key === options.configKey && !originChanged) {
    return {
  ...
  const originCoordinates = configuredOrigin
    ...
    || stateOrigin(db, receiver, cutoff, now);
...
function persistChunk(db, receiverState, cells, touchedTracks, cursor, options, now) {
  const upsertCell = db.prepare(`
    ...
    VALUES (
      @receiverId, @configKey, @cellX, @cellY, @cellZ,
  ...
  db.transaction(() => {
    ...
    updateCursor.run(cursor, now, receiverState.receiverId, options.configKey);
  })();
}
...
export function buildCoverageFromCells(db, rawOptions = {}, aggregation = null, receiverCache = null) {
  ...
  `).all(options.configKey, cutoff, now);
  ...
  for (const receiver of receivers) {
    ...
    const cacheValid = cached
      && cached.configKey === options.configKey
    ...
    if (cacheValid) {
      ...
      const rows = selectCells.all(receiver.receiver_id, options.configKey, cutoff, now);
