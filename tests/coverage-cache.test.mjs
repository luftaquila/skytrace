...
import { createCoverageCache } from "../server/coverage-cache.mjs";
...
test("coverage cache swaps completed async snapshots and defaults to three minutes", async () => {
  ...
  try {
    ...
    assert.equal(first.contentGeneratedAt, "2026-07-23T00:00:00.000Z");

    nowMs += 180000;
    const refreshed = await cache.refresh();
    assert.notStrictEqual(refreshed, first);
    assert.equal(refreshed.build, 2);
    assert.equal(refreshed.contentGeneratedAt, "2026-07-23T00:03:00.000Z");
  } finally {
    await cache.close();
  }
});
...
test("coverage cache serializes refreshes and retains the previous snapshot on failure", async () => {
  ...
  try {
    ...
    assert.equal(cache.state().lastErrorAt != null, true);
    assert.equal("lastError" in cache.state(), false);
