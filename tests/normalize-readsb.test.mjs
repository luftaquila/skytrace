...
test("does not reinterpret removed dump1090-mutability aliases", () => {
  const result = normalizeReadsbPayload({
    now: 1760000000,
    aircraft: [
      {
  ...
  assert.equal(a.altBaro, null);
  assert.equal(a.gs, null);
  assert.equal(a.baroRate, null);
  assert.equal(a.track, 79);
  assert.equal(a.onGround, false);
  assert.equal(b.altBaro, null);
  assert.equal(b.onGround, false);
});

test("uses modern readsb fields while ignoring removed aliases", () => {
  const result = normalizeReadsbPayload({
    now: 1760000000,
    aircraft: [
